...
function buildExpression(node) {
	var converted = convertExpression(node.content);
	return "\n\t" + escapeHTML + "( ( " + converted + " === void 0) ? '' : ( " + converted + " ) )";
}
...
function convertText(text) {
	return text.replace(escapeMatch, "\\\\").replace(newLineMatch, "\\n").replace(quoteMatch, "\\'");
}
