	...
	while (children.length > 1) {
		...
		if (type === "element") {
			...
			if (child.content.match(notWhitespace))
				return setup + buildChildren(children) + ending;
		} else return setup + buildChildren(children) + ending;
	}
	if (foundElements === 1)
...
function buildChildren(children) {
	return "[].concat(" + children.map(buildChild).join(")\n\t\t.concat(") + ")";
}
...
function buildIf(node) {
	return "((" + convertExpression(node.content) + ") ? " + buildChildren(node.children) + " : [])";
}
